...
static float rescale(float t, float min, float max, PlotConfig::Scale::Type type) {
    ...
    case PlotConfig::Scale::Log10:
        return float(log10(ImLerp(min, max, t) / min) / log10(max / min));
    }
    return 0;
}
...
PlotStatus Plot(const char* label, const PlotConfig& conf) {
    ...
    if (conf.values.count > 0) {
        ...
        res_w -= 1;
        float item_count = float(conf.values.count - 1);
            ...
            case PlotConfig::Scale::Log10: {
                ...
                while (start < x_max) {
                    for (int i = 1; i < 10; ++i) {
                        ...
                        if (x < x_min) continue;
                        if (x > x_max) break;
                        float t = float(log10(x / x_min) / log10(x_max / x_min));
        ...
        if (conf.selection.show) {
            ...
            float fSelectionStep = 1.f / item_count;
